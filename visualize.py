...
def cv2_display(size: int, board: np.ndarray, moves, height: int = 800, width: int = 800, background: tuple = (0, 110, 0), adj_cells: set = None):
    """
    ...
    if adj_cells is not None:
        for cell in adj_cells:
            cv2.rectangle(img, (cell[0]*100, cell[1]*100), (cell[0]*100+100, cell[1]*100+100), (0, 0, 255), -1)
    
    # Add pieces (circles)
    for i in range(size):
        for j in range(size):
            if board[i][j] == 1:
                cv2.circle(img, (i*100+50, j*100+50), 40, (255, 255, 255), -1)
            elif board[i][j] == -1:
                cv2.circle(img, (i*100+50, j*100+50), 40, (0, 0, 0), -1)
    # Add possible moves in grey (circles)
    for move in moves_x_y_only:
        cv2.circle(img, (move[0]*100+50, move[1]*100+50), 40, (128, 128, 128), -1)
    # Add grid lines
